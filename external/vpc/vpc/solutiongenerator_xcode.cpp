...
static bool IsCreatedByCustomBuildStep( const CBaseProjectDataCollector *pProjectDataCollector, CFileConfig *pDynamicFileConfig )
{
    for ( int i=pProjectDataCollector->m_Files.First(); i != pProjectDataCollector->m_Files.InvalidIndex(); i=pProjectDataCollector->m_Files.Next(i) )
    {
        ...
        V_snprintf( (char *)sInputFile.Get(), MAX_PATH, "%s/%s", pProjectDataCollector->m_ProjectName.String(), UsePOSIXSlashes( pDynamicFileConfig->m_Filename.String() ) );
        V_RemoveDotSlashes( (char *)sInputFile.Get() );
                        ...
                        if ( sOutputFiles.Length() && !sCustomBuildCommandLine.IsEmpty() )
                        {
                            ...
                            ++m_nIndent;
                            {
                                ...
                                if ( V_IsAbsolutePath( UsePOSIXSlashes( pFileConfig->m_Filename.String() ) ) )
                                    V_snprintf( (char *)sInputFile.Get(), MAX_PATH, "%s", UsePOSIXSlashes( pFileConfig->m_Filename.String() ) );
                                else
                                {
                                    V_snprintf( (char *)sInputFile.Get(), MAX_PATH, "%s/%s", rgchProjectDir, UsePOSIXSlashes( pFileConfig->m_Filename.String() ) );
                                    V_RemoveDotSlashes( (char *)sInputFile.Get() );
                                }
                                ...
                                ++m_nIndent;
                                {
                                    ...
                                    for ( int i = 0; i < outFiles.Count(); i ++ )
                                    {
                                        ...
                                        CBaseProjectDataCollector::DoStandardVisualStudioReplacements( outFiles[i], sInputFile, (char *)sOutputFile.Get(), MAX_PATH );
                                        V_StrSubstInPlace( (char *)sOutputFile.Get(), MAX_PATH, "$(OBJ_DIR)", "${OBJECT_FILE_DIR_normal}", false );

                                        CUtlString sOutputPath;
                                        sOutputPath.SetLength( MAX_PATH );

                                        if ( V_IsAbsolutePath( sOutputFile ) || V_strncmp( outFiles[i], "$", 1 ) == 0 )
                                            V_snprintf( (char *)sOutputPath.Get(), MAX_PATH, "%s", sOutputFile.String() );
                                        else
                                        {
                                            V_snprintf( (char *)sOutputPath.Get(), MAX_PATH, "%s/%s", rgchProjectDir, sOutputFile.String() );
                                            V_RemoveDotSlashes( (char *)sOutputPath.Get() );
                                        }
                                        Write( "\"%s\",\n", sOutputPath.String() );
                                    }
                                }
                                ...
                                V_StrSubstInPlace( (char *)sCommand.Get(), cCommand, "$(OBJ_DIR)", "\"${OBJECT_FILE_DIR_normal}\"", false );
                                V_StrSubstInPlace( (char *)sCommand.Get(), cCommand, ";", ";\\n", false );
                                V_StrSubstInPlace( (char *)sCommand.Get(), cCommand, "\"", "\\\"", false );
                ...
                FOR_EACH_VEC( g_vecPGenerators, iGenerator )
                {
                    ...
                    if ( sPreBuildCommandLine.Length() )
                    {
                        ...
                        ++m_nIndent;
                        {
                            ...
                            CBaseProjectDataCollector::DoStandardVisualStudioReplacements( sPreBuildCommandLine, CFmtStr( "%s/dummy.txt", rgchProjectDir ).Access(), (char *)sCommand.Get(), cCommand );
                            V_StrSubstInPlace( (char *)sCommand.Get(), cCommand, "$(OBJ_DIR)", "\"${OBJECT_FILE_DIR_normal}\"", false );
                            V_StrSubstInPlace( (char *)sCommand.Get(), cCommand, ";", ";\\n", false );
                            V_StrSubstInPlace( (char *)sCommand.Get(), cCommand, "\"", "\\\"", false );
