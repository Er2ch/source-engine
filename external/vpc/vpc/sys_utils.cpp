...
//-----------------------------------------------------------------------------
bool Sys_IsFilenameCaseConsistent( char *pFilename, char *pOutputBuffer, int nOutputBufferSize )
{
	V_strncpy( pOutputBuffer, pFilename, nOutputBufferSize );

	// normalize the provided filename separators
	V_FixSlashes( pFilename );
	V_RemoveDotSlashes( pFilename );

	if ( !Sys_GetActualFilenameCase( pFilename, pOutputBuffer, nOutputBufferSize ) )
		return false;

	if ( !V_strcmp( pFilename, pOutputBuffer ) )
		return true;
